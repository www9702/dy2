 ...
 * 
 * 更新内容: 优化广告报错、优化小岛打卡提示、优化农场三餐、增加乐园抽奖、修复兑换报错
 * 
...
let boxstoptims = "50";       //宝箱广告超指定时间停止 填写注意：1代表1分钟，3代表3分钟
let stoptims = "10";          //看广告超指定时间停止   填写注意：1代表1分钟，3代表3分钟
let appendtims = "10";        //追加广告超指定时间停止 填写注意：1代表1分钟，3代表3分钟
let sizemeasure = "30";       //广告金币低对于指定数值不在看广告   填写注意：30为30金币不再看广告
// ================================== 脚本版本参数 =================================== \\
let scriptVersion = "v2.9";
let update_tines = "2023-08-20";
let update_data = "2023-12-12";   //测试时间
let scriptVersionLatest = "v2.9"; //版本对比
let userCookie = ($.isNode() ? process.env.lekebo_dyjsb2869_Cookie : $.getdata('lekebo_dyjsb2869_Cookie')) || '';
let userList = [];
let userIdx = 0;
...
// ============================================执行项目============================================ \\
                ...
                try {
                    ...
                    if (result == true) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            DoubleLog(`\n ❌ 【${this.index}】天天领金: 搜索奖励溜走，设备问题跳过操作`);
                        } else {
                ...
                try {
                    ...
                    if (result == true) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            DoubleLog(`\n ❌ 【${this.index}】浏览领金: 搜索奖励溜走，设备问题跳过操作`);
                        } else {
                    ...
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            if (timeHours > 5 && timeHours < 10) {
                                ...
                                DoubleLog(`\n ✅ 【${this.index}】出门小岛: 出门记录你的每日此刻活动`);
                                await $.wait(1000);
                                await this.open_square_card(tasktoken,2 * 1000);
                                await $.wait(1000);
                                await this.open_square_carddiary(tasktoken,26,2 * 1000);
                            } else if (timeHours > 16 && timeHours < 20) {
                                DoubleLog(`\n ✅ 【${this.index}】回家小岛: 该时间为晚餐段，执行晚餐任务`);
                                await $.wait(1000);
                                await this.open_square_card(tasktoken,2 * 1000);
                                await $.wait(1000);
                                await this.open_square_carddiary(tasktoken,28,2 * 1000);
                            } else if (timeHours > 20 && timeHours < 6) {
                                DoubleLog(`\n ✅ 【${this.index}】晚安小岛: 该时间为夜宵段，执行夜宵任务`);
                                await $.wait(1000);
                                await this.open_square_card(tasktoken,2 * 1000);
                                await $.wait(1000);
                                await this.open_square_carddiary(tasktoken,28,2 * 1000);
                            ...
                            if(result.data && Array.isArray(result.data)) {
                                for(let i=0; i < result.data.length; i++) {
                                    ...
                                    if (timeHours > 6 && timeHours < 9) {
                                        ...
                                        DoubleLog(`\n ✅ 【${this.index}】午餐礼包: 该时间为午餐段，执行午餐任务`);
                                        await $.wait(1000);
                                        await this.rewardFarmThreeGift(2,2 * 1000);
                                    } else if (timeHours > 17 && timeHours < 21) {
                                        DoubleLog(`\n ✅ 【${this.index}】晚餐礼包: 该时间为晚餐段，执行晚餐任务`);
                                        await $.wait(1000);
                                        await this.rewardFarmThreeGift(3,2 * 1000);
                                    }
                                }
    ...
    async rewardFarmThreeGift(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            if (result.Data.DepositData.Rewards != null) {
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.status_code == 0) {
                            let num = Number(result.data.max_lottery_changes) - Number(result.data.used_lottery_changes) +1;
                            if (result.data.used_lottery_changes >= result.data.max_lottery_changes) {
                                DoubleLog(`\n ❌ 【${this.index}】金币转盘: 每天可完成${result.data.max_lottery_changes}次, 已完成 ${result.data.used_lottery_changes}/${result.data.max_lottery_changes} 次`);
                            } else {
                                DoubleLog(`\n ✅ 【${this.index}】金币转盘: 每天可完成${result.data.max_lottery_changes}次, 已完成 ${result.data.used_lottery_changes}/${result.data.max_lottery_changes} 次`);
                                await $.wait(1000);
                                for (let t = 1; t < num; t++) {
                ...
                try {
                    if (error) {
                        ...
                        if (result.status_code == 0) {
                            ...
                            DoubleLog(`\n ❌ 【${this.index}】转盘奖励: ${result.message}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
