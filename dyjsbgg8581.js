...
let boxstoptims = "10";       //广告金币低于指定数量停止执行 18则是18金币
let stoptims = "50";          //看广告超指定时间停止   填写注意：1代表1分钟，3代表3分钟
// ================================== 脚本版本参数 =================================== \\
let scriptVersion = "v3.2";
let update_tines = "2023-09-04";
let update_data = "2023-12-12";   //测试时间
let scriptVersionLatest = "v3.2"; //版本对比
let userCookie = ($.isNode() ? process.env.lekebo_dyjsb8581_Cookie : $.getdata('lekebo_dyjsb8581_Cookie')) || '';
let userList = [];
let userIdx = 0;
...
// ============================================执行项目============================================ \\
    ...
    async open_video_stop(adid,reqid,amount,opentimes,timeout = 2000) {
        return new Promise((resolve) => {
                ...
                try {
                    ...
                    if (result == true) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            await this.video_more_status(opentimes,2 * 1000);
                        } else {
                            if (timeouts > opentimes) {
                                DoubleLog(`\n ❌ 【${this.index}】温馨提示: 在指定 ${stoptims} 分钟内没执行完成停止`);
                            } else {
                                await this.open_video_stop(adid,reqid,amount,opentimes,2 * 1000);
                            }
                        }
                    } else {
                        if (timeouts > opentimes) {
    ...
    async video_more_status(opentimes,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                ...
                try {
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            await $.wait(1000);
                            await this.one_moread(result.data.aggr_income_id,opentimes,2 * 1000);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】广告追加: ${result.err_tips}`);
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async one_moread(aggrincomeid,opentimes,timeout = 1000) {
        return new Promise((resolve) => {
            let timeouts = Math.round(new Date().getTime()).toString();
            let url = {
                ...
                try {
                    ...
                    if (result == true) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            await this.more_append_two(opentimes,2 * 1000);
                        } else {
                            if (timeouts > opentimes) {
                                DoubleLog(`\n ❌ 【${this.index}】温馨提示: 在指定 ${stoptims} 分钟内没执行完成停止`);
                            } else {
                                await this.one_moread(aggrincomeid,opentimes,2 * 1000);
                            }
                        }
                    } else {
                        if (timeouts > opentimes) {
    ...
    async more_append_two(opentimes,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                ...
                try {
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            await $.wait(1000);
                            await this.two_moread(result.data.aggr_income_id,opentimes,2 * 1000);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】二次追加: ${result.err_tips}`);
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async two_moread(aggrincomeid,opentimes,timeout = 1000) {
        return new Promise((resolve) => {
            let timeouts = Math.round(new Date().getTime()).toString();
            let url = {
                url: `${hostname}/luckycat/aweme/v1/task/done/excitation_ad/one_more?${this.ck[0]}`,
                ...
                try {
                    ...
                    if (result == true) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            await $.wait(1000);
                            await this.more_append_three(opentimes,2 * 1000);
                        } else {
                            if (timeouts > opentimes) {
                               DoubleLog(`\n ❌ 【${this.index}】温馨提示: 在指定 ${stoptims} 分钟内没执行完成停止`);
                            } else {
                                await this.two_moread(aggrincomeid,opentimes,2 * 1000);
                            }
                        }
                    } else {
                        if (timeouts > opentimes) {
    ...
    async more_append_three(opentimes,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                ...
                try {
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            await $.wait(1000);
                            await this.three_moread(result.data.aggr_income_id,opentimes,2 * 1000);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】三次追加: ${result.err_tips}`);
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async three_moread(aggrincomeid,opentimes,timeout = 2000) {
        return new Promise((resolve) => {
            let timeouts = Math.round(new Date().getTime()).toString();
            let url = {
                url: `${hostname}/luckycat/aweme/v1/task/done/excitation_ad/one_more?${this.ck[0]}`,
                    ...
                    if (result == true) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            if (timeouts > opentimes) {
                                ...
                                await this.three_moread(aggrincomeid,opentimes,2 * 1000);
                            }
                        }
                    } else {
                        if (timeouts > opentimes) {
